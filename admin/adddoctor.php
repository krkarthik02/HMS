...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit'])) {
  if (matchPassword()) {
    ...
    $dept = $_POST['dept'];
    $result = mysqli_query($conn, "SELECT * FROM `user_tab` WHERE `username`='$username'");
    $num_rows = mysqli_num_rows($result);
    if ($num_rows) {
      echo "<script>alert('Username Exits/Username Should Not Be Empty');</script>";
    } else {

      $sql = "INSERT INTO `user_tab` (`fname`,`lname`,`username`,`email`,`address`,`city`,`state`,`pincode`,`bloodgroup`,`dob`,`phno`,`password`,`gender`,`image`,`dept_id`,`user_type`) VALUES ('$fname','$lname','$username','$email','$address','$city','$state','$pin','$bgroup','$dob','$phno','$password','$gender','$imagename','$dept','doctor')";
...
<body>
  <div class="reg">
    <div class="container">
      ...
      <div class="content">
        <form action="adddoctor.php" method="POST" class="form" name="form" enctype="multipart/form-data">
